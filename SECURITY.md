...
Send vulnerability reports to `security@neurodock.org`. Encrypted submission is welcome; the maintainer publishes a current PGP key in the repository alongside this file.
...
Do not open a public issue for a security report. Once the issue is resolved and disclosed, the maintainer credits reporters who want credit.
...
First acknowledgement of a report arrives within five working days. Some maintainers operate on a slower async cadence; the maintainer ensures coverage so reports are not stalled by any one maintainer being AFK.
