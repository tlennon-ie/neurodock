...
First release of `neurodock-mcp-guardrail` (Python, PyPI). Per `plan.md` §11
...
References: `plan.md` §8, §11, ADR 0006, `ETHICS.md`,
...
- **`clinical-reviewer` sign-off required** on `src/neurodock_mcp_guardrail/heuristics/` and `_stopwords.py` per ADR 0006 and `ETHICS.md` commitment 3. Council must confirm sign-off captured before tagging.
- **Lived-experience UAT** on the rumination advisory copy — at least one OCD-identified reviewer per `CODEOWNERS` norm. Council confirms before tagging.
