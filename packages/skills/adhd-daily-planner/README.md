...
See `plan.md` §6 (Launch skills #1) for the canonical spec, and `SKILL.md` for the full instructions the LLM follows when this skill activates. The three files under `tests/` are the executable contract — CI replays them against a reference MCP client on every PR.
