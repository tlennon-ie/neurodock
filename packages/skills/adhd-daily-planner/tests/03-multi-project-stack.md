...
The skill must rank by `decided_on` recency, retain the top 5 (`neurodock`, `kipi-system`, `phase-0-rfc`, `mcp-chronometric`, `extension-browser`), and elide the remaining 3 with a count — never naming them.
...
4. `mcp-cognitive-graph.weekly_rollup(project="kipi-system")` → rollup with `next_actions` populated.
...
9. `mcp-task-fractionator.next_one(project="kipi-system")` → confidence 0.74
...
- Exactly five `### <Project name>` sections, in this order: `neurodock`, `kipi-system`, `phase-0-rfc`, `mcp-chronometric`, `extension-browser`.
