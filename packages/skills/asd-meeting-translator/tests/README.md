...
- Prohibited phrases (per `plan.md` §2 voice and the `Do not` block in `SKILL.md`) are tested explicitly: `autistic`, `neurodivergent`, `spectrum`, `executive function`, `superpower`, `crusher`, `smash`, `you got this`, `let's go`, `differently abled`, `clinical`.
