...
- Prohibited phrases (per `plan.md` §2 voice rules and the SKILL.md "Do not" section) are tested explicitly: `rumination`, `anxiety`, `obsessive`, `compulsive`, `spiral`, `loop`, `executive function`, `neurodivergent`, `executive dysfunction`, `intrusive`, plus the general-tone block list (`superpower`, `you got this`, `let's go`).
