...
Quick wrapper for the `mark_session_start` tool on the local
mcp-chronometric server. Captures the user's plain-language framing of
what they are about to do, so later nudges (hyperfocus warnings,
end-of-session reflection) can quote it back unchanged.

Authoritative schema: `packages/mcp-chronometric/schemas/mark_session_start.schema.json`
...
by design (see ADR 0001 — avoids the LLM having to maintain state).
...
- `SESSION_ALREADY_OPEN` — only when the server is configured to reject
  overlaps. The default policy auto-closes (see ADR 0001 open question).
...
- Auto-close behaviour is policy-dependent — check
  `packages/mcp-chronometric/src/neurodock_mcp_chronometric/config.py`
