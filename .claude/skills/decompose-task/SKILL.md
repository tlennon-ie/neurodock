---
name: decompose-task
description: Break a vague goal into 3–12 atomic 5–90 minute tasks with acceptance criteria and dependencies via mcp-task-fractionator. The acceptance bar is "I can start now".
---

# decompose-task

Wrapper for the `decompose` tool on the local mcp-task-fractionator
server. Takes a goal and returns an ordered list of atomic tasks the user
can pick up cold.

Authoritative schema: `packages/mcp-task-fractionator/schemas/decompose.schema.json`.
Design rationale: `docs/decisions/0003-task-fractionator-tool-design.md`
...
4. Does **NOT** persist anything — the caller is responsible for writing
   the result to mcp-cognitive-graph (see the `record-fact` skill) if
  ...
  "tasks": [
    {
      ...
      "description": "Write packages/.../manifesto.md covering ...",
      "estimated_minutes": 45,
      "acceptance_criteria": [
        "File exists at the named path",
        "Section headings match plan.md §2"
      ],
      "dependencies": [],
      "sequence": 1,
      "tags": ["writing"]
...
- Task estimates are LLM-generated and tend optimistic. The
  `hyperfocus-formatter` skill applies a 1.5x buffer when the user shows
  distress signals; this server does not.
