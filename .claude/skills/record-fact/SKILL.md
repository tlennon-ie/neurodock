---
name: record-fact
description: Persist a subject-predicate-object fact in the local cognitive graph. Load this BEFORE calling record_fact to avoid the documented six-retry validation loop.
---

# record-fact

Wrapper around `mcp__neurodock-cognitive-graph__record_fact` that shows
the exact valid input shape up front. The bare tool has noisy validation
errors that have historically taken six attempts to satisfy from cold
(see project memory: `record_fact UX friction`, 2026-05-22). Load this
skill first and call it correctly on the first attempt.

Authoritative schema: `packages/mcp-cognitive-graph/schemas/record_fact.schema.json`
...
Common wrong values: `has_bug`, `is_a`, `causes`, `silently_fails_after_...`.
Free-form predicates land in v0.2 via `type_extensions`. For v0.1.0,
pick the closest verb above — most situations map to `blocked_by`,
`depends_on`, `mentioned_in`, or `tagged`.
...
User says "remember that the Gmail translation bug is blocking LM Studio launch".
