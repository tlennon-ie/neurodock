...
Quick wrapper for `mark_session_end` on the local mcp-chronometric server.
Closes the most-recent open session and returns its final metadata.
...
1. Looks up the most recent open session for the user (no `session_id`
   argument — by design, per ADR 0001).
...
The verbatim `intent` + `summary` pair is the unit consumed by
`weekly_rollup` (see `packages/mcp-cognitive-graph/src/.../weekly_rollup.py`).
Sessions ended without a summary are still included but contribute less
signal. Encourage the user to write a one-line summary even when nothing
"finished" — `"got distracted into refactor; did not finish the planned
migration ADR"` is more useful than no summary at all.
...
- If two sessions are open (policy bug, see ADR 0001 open question), only
