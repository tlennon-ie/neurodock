...
- A proposal would require a major version bump on a server with > 100 known consumers — flag to the council.
- Two specialists propose conflicting tool designs — flag to the council; this is a governance question, not a technical one.
- A clinical-implications tool is being proposed (anything in `mcp-guardrail`) — co-review with `clinical-reviewer`.
