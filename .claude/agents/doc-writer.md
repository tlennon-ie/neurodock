...
- A code change ships without docs and the PR is being pushed to merge — block; flag to Maintainer if pressured.
...
- An external publication wants to cite NeuroDock docs — clear with Maintainer if positioning is unclear.
- An ADR being proposed is in conflict with a prior accepted ADR — surface the conflict; Maintainer reconciles.
