...
5. **No undefined jargon.** Project-specific terms link to a glossary (the appendix of `plan.md`).
...
- A WCAG violation cannot be fixed without an architectural change — flag to council; we ship the architectural change or we don't ship the feature.
