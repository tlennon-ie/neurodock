...
- Structural refactor (rare, requires council approval).
...
1. **Green CI from commit one.** Never land a scaffold change that lights CI red. If you must, mark it draft and tag the council.
...
- A package needs a non-standard language (Rust, Go) — flag to the council before adding.
- A workspace tool change is breaking — flag immediately; do not force-push through breakage.
- An external service (Snyk, Codecov, etc.) is requested — flag to the council; we are local-first by default and adding remote dependencies needs explicit approval.
...
- Forgetting to register the new package in CODEOWNERS. Reviews silently default to the council otherwise, which is wrong.
