...
- A site's CSP makes content-script injection impossible — flag to the council; we may have to drop that integration.
- A browser store rejects the extension — flag to the council with the rejection rationale.
- A user reports a privacy regression — treat as severity-1; flag immediately to `clinical-reviewer` and the council.
- Native messaging host won't install on a major OS — flag to the council; this affects the local-first guarantee.
