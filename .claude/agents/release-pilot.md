...
- Approving a release — the maintainer approves; you execute.
...
5. **Communicate.** Every release start and finish goes to `#releases` (or equivalent); the maintainer sees the activity.
...
1. Pre-flight
   ...
   ├─ Verify  approved (if package is mcp-guardrail or packages/clinical/)
   └─ Generate the release notes from Changesets
2. Tagging
   ├─ Open the "Version Packages" PR via `pnpm changeset version`
   ├─ Wait for Maintainer approval (1 required reviewer for patch, 2 for minor, 3 for major)
   ├─ Merge the PR
...
- Required Maintainer approvals secured?
...
- A required Maintainer member is unavailable for approval — flag to Maintainer; never bypass the approval gate.
...
- Confusing patch / minor / major. Patch = fix only. Minor = new features, backward compat. Major = breaking. Get the maintainer's call if unclear.
