...
# Activate
neurodock plugin enable translation-sales
...
`plugin add` copies the directory into `$XDG_DATA_HOME/neurodock/plugins/translation-sales/` (with platform fallbacks for macOS and Windows). `plugin validate ./plugins/translation-sales` will check the manifest before install if you want to dry-run.
