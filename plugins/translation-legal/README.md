...
If you've cloned the repo, the substrate auto-discovers the plugin at `<repo>/plugins/translation-legal/`. Just restart your client.
...
The CLI's `plugin add` copies the directory into `$XDG_DATA_HOME/neurodock/plugins/translation-legal/` (with platform fallbacks for macOS and Windows). The discovery walker picks it up on next start.
