...
# Activate
neurodock plugin enable translation-hiberno-english
...
`plugin add` copies the directory into `$XDG_DATA_HOME/neurodock/plugins/translation-hiberno-english/` (with platform fallbacks for macOS and Windows). `plugin validate ./plugins/translation-hiberno-english` will check the manifest before install if you want to dry-run.
