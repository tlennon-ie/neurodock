...
- The ecosystem precedent (`this project`, ``, every Claude-skill profile we've inspected) is YAML; switching format costs friction for contributors who already know the convention.
...
1. **How does `extends:` resolve when the named preset is unavailable?** Two clean positions:
   ...
   Recommended: **warn-and-use-defaults**, because the alternative bricks a working install over a missing dependency. Maintainer to confirm before Phase 1 ships.
