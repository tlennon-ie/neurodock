...
- **Informed:** `clinical-reviewer` (no clinical surface in v0.1.0 but Phase 2 may add chronometric-aware nudging), `doc-writer`, `accessibility-auditor`

## Context

`mcp-task-fractionator` is the third substrate server scheduled for Phase 1 (`plan.md` §11), shipping alongside `mcp-chronometric` and `mcp-cognitive-graph` in the `v0.1` developer preview. It supports the user stories in `plan.md` §6, most directly the "Director with ADHD" story: *"voice-dump a vague initiative and get back 7 atomic tasks plus the single next one"* — with the acceptance bar that 80%+ of pilots rate the output "I can start now".
...
3. **No LLM call from inside the server.** Per `plan.md` §4 the LLM boundary is the user's MCP client. The substrate servers do not embed model calls. This is a vendor-neutrality property and a privacy property.
...
- Violates "composable over monolithic" (`plan.md` §1 principle 4) — fractionator and cognitive-graph become hard to swap independently.
...
2. **Should `next_one` consider chronometric `energy_zone` in its choice?** A 60-minute writing task is a worse pick at 4pm `afternoon_dip` than a 15-minute coordination task. v0.1.0 ignores energy zone (purely sequence-based) to keep the dependency graph between substrate servers shallow. Phase 2 enhancement: `next_one` reads `get_time_context()` and weights candidates by tag/energy fit. This would be additive (an optional output field `energy_zone_match: float`); the input schema would not change. Council to confirm before Phase 2 work begins.
