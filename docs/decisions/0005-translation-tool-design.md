...
- **Consulted:** `mcp-server-builder`, `extension-engineer` (parallel — owns the browser-extension scaffold that consumes the same contracts), `eval-curator` (Phase 2 corpus owner), `clinical-reviewer` (advisory on PII handling and consent surfaces)
...
`mcp-translation` is the third substrate MCP server NeuroDock will ship and the engine of Area 2 (communication and translation). Per `plan.md` §7 it launches in Phase 2 alongside a browser extension, and the same prompt library and eval corpus power both surfaces — one MCP server, one extension, one prompt repo, one test harness.
...
5. **PII handling boundary.** The server explicitly does NOT redact. The extension and the user are responsible. Council should confirm this stance — it is the only choice consistent with vendor-neutrality (the server has no PII model) but it pushes a real responsibility upstream.

6. **`brief_meeting.me` is required.** Without a "me" speaker label the four-section partition is ill-defined. The schema returns `ME_REQUIRED` rather than guessing. Council should confirm; the alternative is to default `me = null` and skip the partition (return only `decisions` and `ambiguous_items`), which feels worse.

## Cross-cutting concerns for the council

- **The vendor-boundary rule is now load-bearing across three servers.** `mcp-chronometric`, `mcp-cognitive-graph`, and `mcp-translation` all forbid an embedded LLM SDK. `mcp-task-fractionator` and (Phase 2+) `mcp-guardrail` will inherit the rule by precedent. Council should ratify "no LLM SDK in substrate servers" as substrate-wide doctrine, not a per-server choice.
...
- **The browser extension is a parallel work stream.** The `extension-engineer` agent is scaffolding the extension against these schemas right now. Any change to the schemas before v0.1.0 tag must be propagated. Council should designate a single owner of "schema-as-source-of-truth" for the duration of Phase 2.
- **Language packs and domain packs are contribution lanes (plan.md §7).** The plugin manifest schema for `type: language-pack` is referenced but unspecified. A short ADR-0006 sketch is needed before external contributors can land packs.
