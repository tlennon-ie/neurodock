---
title: CLI
description: The neurodock command-line interface. Eighteen commands across seven groups, shipped in @neurodock/cli 0.5.0.
---

:::note[CLI status]
`@neurodock/cli` `0.5.0` is shipped to npm. Eighteen commands across seven groups (`init`, `install-all`, `examples`, `doctor`, `validate`, `update`, `sync`, `uninstall`, `host install/uninstall`, `profile show/validate`, `plugin add/remove/list/enable/disable/validate`). The CLI is the lowest-friction onboarding path: `npx --yes @neurodock/cli@latest install-all` should take a fresh install from zero to a working substrate in under five minutes.
:::
...
| Flag | Meaning |
|---|---|
| `-v, --version` | Print the CLI version (`0.5.0`). |
