...
A plugin declaring any other SPDX identifier fails to load. See [Write a plugin: identity, license, and trust](/contribute/write-a-plugin/#3-add-identity-license-and-trust) for the rationale.
...
| `signature` | string or null | `null` | Reserved. Verified-tier signing is future work — see [ROADMAP.md](https://github.com/tlennon-ie/neurodock/blob/main/ROADMAP.md). |
...
Until a dedicated `neurodock validate <plugin-dir>` CLI subcommand lands (planned — see [ROADMAP.md](https://github.com/tlennon-ie/neurodock/blob/main/ROADMAP.md)), validate with any JSON Schema tool against `packages/core/schemas/plugin.schema.json`:
