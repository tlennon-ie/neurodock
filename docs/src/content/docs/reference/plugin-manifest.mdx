...
> **Status note.** The schema file is owned by the `mcp-architect` agent and arrives alongside ADR 0007. Until that ADR merges, this reference reflects the agreed shape from `plan.md` §5 and the doc-writer-architect contract; field names may shift by name but not by intent before `v0.1.0` is tagged.
...
Drives two behaviours: auto-activation against the user's profile, and review routing (a plugin tagged `adhd` requires an ADHD-lived-experience reviewer before promotion to `reviewed`). `audhd` is its own value, not `["adhd", "asd"]`. The lived experience differs from either alone.
...
trust:
  ...
  reviewed_by: []                # Optional list of council-registered reviewer handles
```
...
| `reviewed_by` | string[] | `[]` | Council-registered reviewer handles who have signed off. |
