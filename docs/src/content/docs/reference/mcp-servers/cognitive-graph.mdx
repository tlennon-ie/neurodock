...
Forgiving on input: a user typing `"kipi"` retrieves `"kipi-system"`. Resolution falls through `exact → alias → fuzzy → embedding`. The output carries a `resolution.method` and `resolution.score` so skills can say "I think you meant X" when the method is `fuzzy` or `embedding`.
