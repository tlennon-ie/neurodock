...
`mcp-chronometric` externalises time and session awareness so the LLM does not have to maintain that state across turns. It is the first substrate server scheduled for Phase 1 and the precedent-setting design for every later substrate server.
