...
:::caution[How firing works]
Every firing surfaces a plain-language reason and a non-empty list of override options. Nothing is silently blocked, rewritten, or hidden. You always get to keep going if you want to — see the flow below.
:::
...
All three detectors are live as of `mcp-guardrail` `v0.0.3`. The schemas remain locked at `v0.1.0` per [ADR 0006](/decisions/0006-guardrail/); the runtime now implements every detector the schema reserves.
