...
The `locales/messages.json` block is for the substrate's own UI strings as they relate to this language (notice text, consent prompts, error messages from the server). UI strings follow the ICU MessageFormat convention used by the docs site and the browser extension — see .
...
-  for the broader communication-layer architecture and the four contribution lanes.
