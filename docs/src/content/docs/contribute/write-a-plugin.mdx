...
:::tip[Copy a working template]
The fastest path is to copy an existing plugin and edit. Use [`plugins/example-skill-pomodoro/`](https://github.com/tlennon-ie/neurodock/tree/main/plugins/example-skill-pomodoro) for a skill-type plugin, or browse [`plugins/`](https://github.com/tlennon-ie/neurodock/tree/main/plugins) for translation-pack examples (`translation-hiberno-english`, `translation-german-directness`, `translation-japanese-keigo`, `translation-legal`, `translation-healthcare`, etc.). Every plugin in `plugins/` is a working example.
:::
...
The substrate's plugin schema lives at `packages/core/schemas/plugin.schema.json`. A dedicated `neurodock validate <plugin-dir>` CLI subcommand is future work (see [Future work](#future-work) below); until then, validate using any JSON Schema tool:
...
A clean validation prints `valid`. A failed validation prints a path to the offending field. Raw JSON Schema validation is the supported path today; a dedicated plugin-directory CLI subcommand is tracked under [Future work](#future-work) below.
...
Verified-tier signing is future work alongside the federated registry (see [Future work](#future-work)). The signing process the maintainer will operate:
...
A small set of plugin-ecosystem items is intentionally out of scope for v0.2.1. They are tracked on [`ROADMAP.md`](https://github.com/tlennon-ie/neurodock/blob/main/ROADMAP.md) §Later under "Plugin marketplace" and elsewhere:
...
- **`#plugins` Discord channel** is planned as a live channel — see [Future work](#future-work). Until it exists, use GitHub Discussions; we will link the invite from this page when it goes live.
